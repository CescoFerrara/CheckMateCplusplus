...
namespace Chess
{
  ...
  public:
    ...
    void deletePc(std::pair<char, char> position);
    ...
    // Returns piece location given character representation if it exists on board
    // if the piece does not exist on the board it returns '-' '-'
    //only works every time for kings
    std::pair<char,char> get_piece(char piece) const;
    ...
    public:
      ...
      iterator(Piece* initial) : piece(initial) { }
      
      std::pair<char, char> getLocation();
